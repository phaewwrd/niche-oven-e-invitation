...
export function RsvpForm({ eventId, primaryColor = "#000" }: RsvpFormProps) {
    ...
    if (isSubmitted) {
        return (
            <div className="text-center p-12 bg-white/50 backdrop-blur-sm rounded-[2rem] border-2 border-dashed border-primary/20 animate-in zoom-in duration-500">
                <div className="flex justify-center mb-6">
                    <CheckCircle2 className="w-16 h-16 text-green-500" />
                </div>
                <h3 className="text-2xl font-serif italic mb-2">Thank You!</h3>
                <p className="text-muted-foreground">Your response has been recorded beautifully.</p>
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 text-left max-w-lg mx-auto bg-white/40 backdrop-blur-md p-10 rounded-[2.5rem] border border-white/60 shadow-xl">
            <div className="space-y-6">
                <div className="space-y-3">
                    ...
                    <Input
                        ...
                        className="rounded-2xl border-white/50 bg-white/50 h-14 px-6 focus:ring-primary/20"
                    />
                ...
                <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex h-14 bg-white/50 rounded-2xl p-1 border border-white/50">
                            <button
                                type="button"
                                onClick={() => setValue("isAttending", true)}
                                className={`flex-1 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${isAttending ? 'bg-primary text-white shadow-md' : 'text-primary/40 hover:text-primary'}`}
                            >
                                Yes
                            </button>
                            <button
                                ...
                                className={`flex-1 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${!isAttending ? 'bg-destructive text-white shadow-md' : 'text-primary/40 hover:text-primary'}`}
                            >
                                No
                            </button>
                        </div>
                ...
                <div className="space-y-3">
                    ...
                    <Textarea
                        ...
                        className="rounded-2xl border-white/50 bg-white/50 min-h-[120px] px-6 py-4 focus:ring-primary/20 resize-none font-sans text-sm"
                    />
                </div>
            </div>
            ...
            <Button
                ...
                className="w-full h-16 rounded-2xl text-xs font-black uppercase tracking-[0.3em] shadow-xl hover:brightness-110 active:scale-[0.98] transition-all text-white"
            >
