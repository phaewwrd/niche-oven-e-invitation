    ...
    return (
        ...
        >
            ...
            <header className="relative h-screen w-full overflow-hidden">
                ...
                <div className="absolute top-12 right-12 text-right z-10 select-none pointer-events-none">
                    <div className="flex flex-col items-end leading-none">
                        <span className="text-8xl sm:text-9xl font-black opacity-90 drop-shadow-2xl">{formatDay(event.eventDate)}</span>
                        <span className="text-8xl sm:text-9xl font-black opacity-90 drop-shadow-2xl">{formatMonth(event.eventDate)}</span>
                        <span className="text-8xl sm:text-9xl font-black opacity-90 drop-shadow-2xl">{formatYearShort(event.eventDate)}</span>
                    </div>
                </div>

                <div className="absolute bottom-20 left-12 z-10 max-w-xl">
                    <h1 className="text-5xl sm:text-7xl font-serif font-black italic uppercase leading-none tracking-tighter">
                        {event.groomName} <br />
            ...
            <section className="py-32 px-12 bg-white text-black">
                <div className="max-w-xl mx-auto flex gap-12 items-start">
                    <div className="flex flex-col leading-none font-black text-3xl opacity-20">
                        <span>{formatDay(event.eventDate)}</span>
                        <span>{formatMonth(event.eventDate)}</span>
                        <span>{formatYearShort(event.eventDate)}</span>
                    </div>
                    <div>
                        <h2 className="text-4xl font-serif font-black italic uppercase mb-8 tracking-widest">INVITATION</h2>
            ...
            <section className="py-20 px-6 bg-white text-black">
                <div className="max-w-md mx-auto bg-black text-white p-12 rounded-[3.5rem] shadow-2xl overflow-hidden relative group">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full blur-3xl" />
                    <h2 className="text-4xl font-serif font-black italic uppercase text-center mb-8 tracking-tighter">LOCATION</h2>
                    <div className="text-center space-y-4 mb-8">
                        <p className="text-lg font-serif italic opacity-60 px-4">{event.locationText}</p>
                        <div className="text-5xl font-black tracking-tighter">15:00</div>
                    </div>
                    {event.googleMapsUrl && (
                        <a
            ...
            <section className="py-32 bg-white text-black px-12">
                <div className="max-w-2xl mx-auto">
                    <h2 className="text-4xl font-serif font-black italic uppercase mb-16 tracking-widest text-center">PROGRAM</h2>

                    <div className="relative border-l-2 border-black/10 ml-8 pl-12 space-y-16">
                        {sortedSchedules.map((item, idx) => (
                            <div key={idx} className="relative group">
                                <div className="absolute -left-[3.4rem] top-2 w-4 h-4 bg-black rounded-full ring-8 ring-white transition-transform group-hover:scale-125" />
                                <div className="flex flex-col">
                                    <span className="text-xl font-black tracking-tighter mb-1">{formatTime(item.time)}</span>
                                    <span className="text-2xl font-serif italic opacity-60 leading-tight">{item.title}</span>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            {event.dressCodeColors && event.dressCodeColors.length > 0 && (
                <section className="py-20 px-6 bg-white text-black">
                    <div className="max-w-md mx-auto bg-black text-white p-12 rounded-[3.5rem] shadow-2xl">
                        <h2 className="text-3xl font-serif font-black italic uppercase text-center mb-8 tracking-widest">DRESS CODE</h2>
                        <p className="text-center text-sm font-serif italic opacity-60 mb-10">
                        ...
                        <div className="flex justify-center gap-4">
                            {event.dressCodeColors.map((color, idx) => (
                                <div key={idx} className="flex flex-col items-center gap-2">
                                    <div
                                        className="w-14 h-14 rounded-full border-2 border-white/20 shadow-lg transform transition-transform hover:scale-110"
            ...
            <section className="py-32 px-12 bg-white text-black text-center">
            ...
            {/* FOOTER */}
            <footer className="py-24 px-12 bg-white text-black text-center border-t border-black/5">
                <div className="flex flex-col items-center gap-4">
                    <Heart className="w-6 h-6 opacity-20" />
                    <p className="text-2xl font-serif italic opacity-40">Ready to celebrate with</p>
                    <h2 className="text-4xl font-serif font-black uppercase tracking-tighter italic">
